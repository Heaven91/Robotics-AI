...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        # Add code here
        # pay attention to here, the trace is divided into four parts, the left and rigth circle parts,
        # the top straight line and bottom straight line. for the cicle parts, cte is the shortest
        # path to the circle arc, so we cauculate the distance to circle center, then minus the radius,
        # so the get the error between car and trace, for straight parts, cte
        # is gap for y coordianates.
        if self.x <= radius:
            cte = sqrt((self.x - radius) ** 2 +
                       (self.y - radius) ** 2) - radius
        elif self.x >= 3 * radius:
            cte = sqrt((self.x - 3 * radius) ** 2 +
                       (self.y - radius) ** 2) - radius
...
pl.plot(err[1], err[2], 'o')
pl.xlim(min(err[1]) - 1, max(err[1]) + 1)  # set axis limits
pl.ylim(min(err[2]) - 1, max(err[2]) + 1)
pl.show()
