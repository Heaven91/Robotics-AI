...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # update Gaussian
            # calcualte the weight of each particle, if error_bearing is samll, which means the particle is approximate to 
            # our ground turth particle, the the value of gaussian functio is big, this particle get a higher weigth. ecah 
            # weigth is calculated by considering four landmarks together
    ...
    def sense(self, flag): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            ...
            if flag:
                angle = ( 2*pi + atan2(delta_y, delta_x)) % (2*pi)  - self.orientation + random.gauss(0, self.bearing_noise)
            else:
                angle = ( 2*pi + atan2(delta_y, delta_x)) % (2*pi)  - self.orientation
            Z.append(angle)
