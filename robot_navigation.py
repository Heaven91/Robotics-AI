...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000):
    ...
    err = 0.0
    N = 0
    pd = []  # used for store the actual robot trajectory under the PD control
    pd.append([myrobot.x, myrobot.y])
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # calculate the motion vector and robot position vector
        ...
        # calculate segmentation threshold and cte
        cte = (R_y * delta_x - R_x * delta_y) / (delta_x ** 2 + delta_y ** 2)
        u = (R_x * delta_x + R_y * delta_y) / (delta_x ** 2 + delta_y ** 2)

        # if u bigger than 1, then entrance the next straight line
        if u > 1:
            index += 1
        # ENTER CODE HERE
    ...
    return [myrobot.check_goal(goal), myrobot.num_collisions, myrobot.num_steps, pd]

# this function is used for visualize the path


def visualize_path(path):
    # path should be given as a list format, every element in \
    # list is a list
    x = []
    y = []
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
    ...
    path.astar()
    path_star = path.path  # original path
    path.smooth(weight_data, weight_smooth)
    spath = path.spath  # smooth path
    PD_control = run(grid, goal, path.spath, [p_gain, d_gain])
    real_path = PD_control[-1]  # robot actual path
    visualize_path([path_star, spath, real_path])
    return PD_control
